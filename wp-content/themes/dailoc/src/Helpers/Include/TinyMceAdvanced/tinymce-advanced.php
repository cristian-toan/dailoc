...
if(!class_exists('Tinymce_Advanced')) :
    ...
    class Tinymce_Advanced {
        ...
        public function __construct() {
            ...
            register_activation_hook(__FILE__, [$this, 'check_plugin_version']);
            
            add_action('plugins_loaded', [$this, 'set_paths'], 50);
            ...
            if(is_admin()) {
                ...
                add_action('before_wp_tiny_mce', [$this, 'show_version_warning']);
            }
            
            add_filter('wp_editor_settings', [$this, 'disable_for_editor'], 10, 2);
            
            add_filter('mce_buttons', [$this, 'mce_buttons_1'], 999, 2);
            add_filter('mce_buttons_2', [$this, 'mce_buttons_2'], 999);
            add_filter('mce_buttons_3', [$this, 'mce_buttons_3'], 999);
            add_filter('mce_buttons_4', [$this, 'mce_buttons_4'], 999);
            
            add_filter('tiny_mce_before_init', [$this, 'mce_options']);
            add_filter('mce_external_plugins', [$this, 'mce_external_plugins'], 999);
            add_filter('tiny_mce_plugins', [$this, 'tiny_mce_plugins'], 999);
            add_action('after_wp_tiny_mce', [$this, 'after_wp_tiny_mce']);
        }
        ...
        public function disable_for_editor($settings, $editor_id) {
            ...
            if(!empty($this->admin_settings['disabled_editors'])) {
                ...
                if(is_admin()) {
                    ...
                    } else if(in_array('rest_of_wpadmin', $disabled_editors, true)) {
                        $this->disabled_for_editor = true;
                    }
                } else if(in_array('on_front_end', $disabled_editors, true)) {
                    $this->disabled_for_editor = true;
                }
            }
        ...
        public function set_paths() {
            if(!defined('TADV_URL')) {
                define('TADV_URL', get_stylesheet_directory_uri() . "/src/Helpers/Include/tinymce-advanced/");
            }
            
            if(!defined('TADV_PATH')) {
                define('TADV_PATH', get_stylesheet_directory() . "/src/Helpers/Include/tinymce-advanced/");
            }
        }
        ...
        public function enqueue_scripts($page) {
            if('settings_page_tinymce-advanced' == $page) {
                wp_enqueue_script('tadv-js', get_stylesheet_directory_uri() .  '/src/Helpers/Include/TinyMceAdvanced/js/tadv.js', ['jquery-ui-sortable'], '4.0', true);
                wp_enqueue_style('tadv-mce-skin', includes_url('js/tinymce/skins/lightgray/skin.min.css'), [], '4.0');
                wp_enqueue_style('tadv-css', get_stylesheet_directory_uri() . '/src/Helpers/Include/TinyMceAdvanced/css/tadv-styles.css', ['editor-buttons'], '4.0');
                
                add_action('admin_footer', [$this, 'load_mce_translation']);
            }
        }
        ...
        public function load_settings() {
            ...
            $this->admin_options = !empty($this->admin_settings['options']) ? explode(',', $this->admin_settings['options']) : [];
            
            if($this->user_settings === false) {
                $this->user_settings = $this->get_default_user_settings();
            }
            
            $this->options   = !empty($this->user_settings['options']) ? explode(',', $this->user_settings['options']) : [];
            $this->plugins   = !empty($this->user_settings['plugins']) ? explode(',', $this->user_settings['plugins']) : [];
            $this->toolbar_1 = !empty($this->user_settings['toolbar_1']) ? explode(',', $this->user_settings['toolbar_1']) : [];
            $this->toolbar_2 = !empty($this->user_settings['toolbar_2']) ? explode(',', $this->user_settings['toolbar_2']) : [];
            $this->toolbar_3 = !empty($this->user_settings['toolbar_3']) ? explode(',', $this->user_settings['toolbar_3']) : [];
            $this->toolbar_4 = !empty($this->user_settings['toolbar_4']) ? explode(',', $this->user_settings['toolbar_4']) : [];
        ...
        private function check_admin_setting($setting) {
            ...
            if(strpos($setting, 'enable_') === 0) {
                $disabled_editors = !empty($this->admin_settings['disabled_editors']) ? explode(',', $this->admin_settings['disabled_editors']) : [];
        ...
        public function after_wp_tiny_mce() {
            ...
            <script>
                !function (a, b) {
                    "undefined" != typeof a && "undefined" != typeof b && a(function () {
                        b.addButton("sofbg-axcell", "&para;", function (b, c) {
        ...
        public function mce_external_plugins($mce_plugins) {
            ...
            $plugpath    = get_stylesheet_directory_uri()  . '/src/Helpers/Include/TinyMceAdvanced/mce/';
        ...
        private function parse_buttons($toolbar_id = false, $buttons = false) {
            ...
            if(is_array($buttons)) {
                $_buttons = array_map([@$this, 'filter_name'], $buttons);
        ...
        private function save_settings($all_settings = null) {
            ...
            for($i = 1; $i < 5; $i++) {
                ...
                } else if(!empty($_POST[$toolbar_name]) && is_array($_POST[$toolbar_name])) {
            ...
            } else if(!empty($_POST['options']) && is_array($_POST['options'])) {
            ...
            } else if(!empty($_POST['options']['menubar'])) {
                $plugins = ['anchor', 'code', 'insertdatetime', 'nonbreaking', 'print', 'searchreplace', 'table', 'visualblocks', 'visualchars'];
            } else {
                $plugins = [];
            }
        ...
        public function settings_page() {
            ...
            include_once(__DIR__ . '/tadv_admin.php');
        }
        
        public function add_menu() {
            add_options_page('TinyMCE Advanced', 'TinyMCE Advanced', 'manage_options', 'tinymce-advanced', [$this, 'settings_page']);
        }
